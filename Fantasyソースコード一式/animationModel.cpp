...
XMMATRIX AssimpToXMMATRIX(const aiMatrix4x4& matrix)
{
	return XMMATRIX(
...
XMMATRIX AnimationModel::GetBoneMatrix(const std::string& boneName)
{
	...
	if (it == m_Bone.end())
	{
		throw std::runtime_error("Bone not found: " + boneName);
	}

	const BONE& bone = it->second;
	XMMATRIX boneMatrix = AssimpToXMMATRIX(bone.OffsetMatrix);

	const aiNode* node = m_AiScene->mRootNode->FindNode(boneName.c_str());
	if (!node)
	{
		throw std::runtime_error("Bone node not found: " + boneName);
	}

	XMMATRIX parentTransform = XMMatrixIdentity();
	while (node)
	{
		parentTransform = AssimpToXMMATRIX(node->mTransformation) * parentTransform;
		node = node->mParent;
	}

	return boneMatrix * parentTransform;
}


XMFLOAT3 AnimationModel::GetBoneWorldPosition(const std::string& boneName)
{
	auto boneMatrix = GetBoneMatrix(boneName); // ボーンの行列を取得
	XMVECTOR bonePos = XMVector3TransformCoord(XMVectorZero(), boneMatrix); // ワールド座標変換

	XMFLOAT3 result;
	XMStoreFloat3(&result, bonePos);
	return result;
}
							...
							float BlendRatio)
{
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		for (unsigned int v = 0; v < mesh->mNumVertices; v++)
		{
			...
			deformVertex->Normal = mesh->mVertices[v];
