...
void Renderer::Init()
{
	...
	// デバイスの設定の引数第一引数を何かに変えるとRTXのGeForceが使える
	...
	rasterizerDesc.CullMode = D3D11_CULL_BACK; //バックフェースカリング
	...
	// Setup Platform/Renderer backends 自分のレンダラーCPPで作成している変数入れている
...
void Renderer::Uninit()
{
	...
	if (m_StencilWrite) m_StencilWrite->Release();
	if (m_StencilRead) m_StencilRead->Release();

	if (m_BlendState) m_BlendState->Release();
	if (m_BlendStateAdd) m_BlendStateAdd->Release();//加算合成ブレンドステート
	if (m_BlendStateATC) m_BlendStateATC->Release();
	if (m_BlendStateMask) m_BlendStateMask->Release();

	if (m_RasterizerStateCullBack) m_RasterizerStateCullBack->Release();
	if (m_RasterizerStateCullNone) m_RasterizerStateCullNone->Release();
...
void Renderer::Begin()
{
	float clearColor[4] = { 0.9f, 0.9f, 1.0f, 1.0f };
